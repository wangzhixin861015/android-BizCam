...
import jp.co.cyberagent.android.gpuimage.GPUImageCricleLightFilter;
...
import jp.co.cyberagent.android.gpuimage.GPUImageHighBcnetechFilter;
import jp.co.cyberagent.android.gpuimage.GPUImageHighlightFilter;
...
import jp.co.cyberagent.android.gpuimage.GPUImageLevelsMaxFilter;
...
import jp.co.cyberagent.android.gpuimage.GPUImagePartExposureFilter;
import jp.co.cyberagent.android.gpuimage.GPUImagePartSaturationFilter;
...
import jp.co.cyberagent.android.gpuimage.GPUImageShadowBcnetechFilter;
import jp.co.cyberagent.android.gpuimage.GPUImageShadowFilter;
...
import jp.co.cyberagent.android.gpuimage.GPUImageWhiteBalanceTintFilter;
import jp.co.cyberagent.android.gpuimage.GPUImageWhiteBalanceTintNewFilter;

public class GPUImageFilterTools {
    public static final int SHOW=1;//显示
    public static final int SRC=0;//原图
    public static final int UN_SHOW=-1;//不显示
    ...
    public static GPUImageFilter createFilterForType(final Context context, final FilterType type,final int ...drawarl) {
        switch (type) {
            case CONTRAST:
            ...
            case SHADOW:
                return new GPUImageShadowFilter(0.0f, 1.0f);
            case HIGHLIGHT:
            ...
            case WHITE_BALANCE_NEW:
                return new GPUImageWhiteBalanceNewFilter(0.0f, 0.0f);
            case WHITE_BALANCE_TINT:
                return new GPUImageWhiteBalanceTintFilter(5000.0f, 0.0f);
            case WHITE_BALANCE_TINT_NEW:
                return new GPUImageWhiteBalanceTintNewFilter(0.0f, 0.0f);
            ...
            case TONE_CURVE:
                ...
                toneCurveFilter.setFromCurveFileInputStream(
                        context.getResources().openRawResource(drawarl[0]));
            ...
            case LOOKUP_AMATORKA:
                ...
                amatorka.setBitmap(BitmapFactory.decodeResource(context.getResources(), drawarl[0]));
                return amatorka;
            case GAUSSIAN_BLUR:
...
//                levelsFilter.setMin(0.0f, 1.0f, 1.0f);
                return new GPUImageLevelsFilter();
            case LEVELS_FILTER_MAX:
                GPUImageLevelsMaxFilter levelsMaxFilter = new GPUImageLevelsMaxFilter();
//                levelsMaxFilter.setMin(0.0f, 1.0f, 1.0f);
                return new GPUImageLevelsMaxFilter();
            case HALFTONE:
            ...
            case PART_EXPOSURE:
                return new GPUImagePartExposureFilter(0f);
            case PART_SATURATION:
                return new GPUImagePartSaturationFilter(0f);

            case CRICLE_LIGHT:
                return new GPUImageCricleLightFilter();
            case ATMOS_PHERE:
                return new GPUImageAtmosphereFilter();

            case DEFINITION:
                return new GPUImageDefinitionFilter();

            case SHADOW_BCNETECH:
                return new GPUImageShadowBcnetechFilter();

            case HIGHLIGHT_BCNETECH:
                return new GPUImageHighBcnetechFilter();
            default:
    ...
    private static GPUImageFilter createBlendFilter(Context context, Class<? extends GPUImageTwoInputFilter> filterClass,int ...drawarl) {
        try {
            GPUImageTwoInputFilter filter = filterClass.newInstance();
            filter.setBitmap(BitmapFactory.decodeResource(context.getResources(), drawarl[0]));
            return filter;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public enum FilterType {
        ...
        SATURATION, EXPOSURE, HIGHLIGHT_SHADOW, MONOCHROME, OPACITY, RGB, WHITE_BALANCE,WHITE_BALANCE_NEW,WHITE_BALANCE_TINT,WHITE_BALANCE_TINT_NEW, VIGNETTE, TONE_CURVE, BLEND_COLOR_BURN, BLEND_COLOR_DODGE, BLEND_DARKEN, BLEND_DIFFERENCE,HIGHLIGHT,SHADOW,
        BLEND_DISSOLVE, BLEND_EXCLUSION, BLEND_SOURCE_OVER, BLEND_HARD_LIGHT, BLEND_LIGHTEN, BLEND_ADD, BLEND_DIVIDE, BLEND_MULTIPLY, BLEND_OVERLAY, BLEND_SCREEN, BLEND_ALPHA,
        BLEND_COLOR, BLEND_HUE, BLEND_SATURATION, BLEND_LUMINOSITY, BLEND_LINEAR_BURN, BLEND_SOFT_LIGHT, BLEND_SUBTRACT, BLEND_CHROMA_KEY, BLEND_NORMAL, LOOKUP_AMATORKA,
        GAUSSIAN_BLUR, CROSSHATCH, BOX_BLUR, CGA_COLORSPACE, DILATION, KUWAHARA, RGB_DILATION, SKETCH, TOON, SMOOTH_TOON, BULGE_DISTORTION, GLASS_SPHERE, HAZE, LAPLACIAN, NON_MAXIMUM_SUPPRESSION,
        SPHERE_REFRACTION, SWIRL, WEAK_PIXEL_INCLUSION, FALSE_COLOR, COLOR_BALANCE, LEVELS_FILTER_MIN, LEVELS_FILTER_MAX,BILATERAL_BLUR, HALFTONE, TRANSFORM2D,
        PART_EXPOSURE ,PART_SATURATION,CRICLE_LIGHT,ATMOS_PHERE,DEFINITION,SHADOW_BCNETECH,HIGHLIGHT_BCNETECH
    }
    ...
    public static class FilterAdjuster {
        ...
        public FilterAdjuster(final GPUImageFilter filter) {
            ...
            } else if (filter instanceof GPUImageWhiteBalanceTintFilter) {
                adjuster = new WhiteBalanceTintAdjuster().filter(filter);
            } else if (filter instanceof GPUImageWhiteBalanceNewFilter) {
                adjuster = new WhiteBalanceNewAdjuster().filter(filter);
            } else if (filter instanceof GPUImageWhiteBalanceTintNewFilter) {
                adjuster = new WhiteBalanceTintNewAdjuster().filter(filter);
            ...
            } else if (filter instanceof GPUImageLevelsMaxFilter) {
                adjuster = new LevelsMaxAdjuster().filter(filter);
            ...
            } else if (filter instanceof GPUImagePartExposureFilter) {
                adjuster = new PartExposureAdjuster().filter(filter);
            } else if (filter instanceof GPUImagePartSaturationFilter) {
                adjuster = new PartSaturationAdjuster().filter(filter);
            }else if (filter instanceof GPUImageCricleLightFilter) {
                adjuster = new CricleLightAdjuster().filter(filter);
            }else if (filter instanceof GPUImageAtmosphereFilter) {
                adjuster = new AtmosphereAdjuster().filter(filter);
            }else if (filter instanceof GPUImageDefinitionFilter) {
                adjuster = new DefinitionAdjuster().filter(filter);
            }else if (filter instanceof GPUImageShadowBcnetechFilter) {
                adjuster = new ShadowBcnetechAdjuster().filter(filter);
            }else if (filter instanceof GPUImageHighBcnetechFilter) {
                adjuster = new HighBcnetechAdjuster().filter(filter);
            }
        ...
        private abstract class Adjuster<T extends GPUImageFilter> {
            ...
            public abstract void adjust(int percentage,int type);

            protected float range100(final int percentage, final float start, final float end) {
            ...
            protected float changType(final int type){
                switch (type){
                    case SHOW:
                        return 1f;
                    case SRC:
        ...
        private class SharpnessAdjuster extends Adjuster<GPUImageSharpenFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setSharpness(range(percentage, -1.0f, 1.0f));
            }
        ...
        private class ContrastAdjuster extends Adjuster<GPUImageContrastFilter> {
            ...
            public void adjust(final int percentage) {
         /*       if(percentage<100){
                    getFilter().setContrast(range(percentage, 0.5f, 1.0f));
                }
                else if(percentage>100){
                    getFilter().setContrast(range(percentage, 0.0f, 2.2f));
                }else{
                    getFilter().setContrast(range(percentage, 0.0f, 2.0f));
                }*/
                getFilter().setContrast(range(percentage, -0.24f, 2.2f));
        ...
        private class BrightnessAdjuster extends Adjuster<GPUImageBrightnessFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setBrightness(range(percentage, -0.3f, 0.3f));
            }
            @Override
            public void adjust(int percentage, int type) {
        ...
        private class HighlightAdjuster extends Adjuster<GPUImageHighlightFilter> {
            @Override
            public void adjust(final int percentage) {
               // getFilter().setShadows(0);

                getFilter().setHighlights(1-range(percentage, 0.0f, 1.0f));
            }
            @Override
            public void adjust(int percentage, int type) {
        ...
        private class WhiteBalanceTintAdjuster extends Adjuster<GPUImageWhiteBalanceTintFilter> {
            @Override
            public void adjust(final int percentage) {
        ...
        private class WhiteBalanceNewAdjuster extends Adjuster<GPUImageWhiteBalanceNewFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setTemperature(percentage-100);
            }
            @Override
            public void adjust(int temperature, int tint) {

                getFilter().setTemperature(temperature-100);

                getFilter().setTint(tint-100);
        ...
        private class WhiteBalanceTintNewAdjuster extends Adjuster<GPUImageWhiteBalanceNewFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setTint(percentage-100);
            }
            @Override
            public void adjust(int percentage, int type) {
        ...
        private class LevelsMinMidAdjuster extends Adjuster<GPUImageLevelsFilter> {
            @Override
            public void adjust(int percentage) {
                getFilter().setMin(0.0f,range100(percentage, 0.0f, 1.0f), 1.0f);
            }
            @Override
            public void adjust(int percentage, int type) {
        ...
        private class LevelsMaxAdjuster extends Adjuster<GPUImageLevelsMaxFilter> {
            @Override
            public void adjust(int percentage) {
                getFilter().setMin(0.0f, 1.0f, range100(percentage, 0.0f, 1.0f));
            }
            @Override
            public void adjust(int percentage, int type) {
        ...
        private class PartExposureAdjuster extends Adjuster<GPUImagePartExposureFilter>{
            @Override
            public void adjust(int percentage) {
                getFilter().setExposure(changType(percentage));
            }
            @Override
            public void adjust(int percentage, int type) {
        ...
        private class PartSaturationAdjuster extends Adjuster<GPUImagePartSaturationFilter>{
            @Override
            public void adjust(int percentage) {
                getFilter().setExposure(changType(percentage));
            }
        ...
        private class CricleLightAdjuster extends Adjuster<GPUImageCricleLightFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setExposure(range(percentage, -1.0f, 1.0f));
                getFilter().setExposure2(range(percentage, -1.0f, 1.0f));
            }
            @Override
            public void adjust(int percentage, int type) {
        ...
        private class AtmosphereAdjuster extends Adjuster<GPUImageAtmosphereFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setExposure(range(percentage, -1.0f, 1.0f));
                getFilter().setExposure2(range(percentage, -1.0f, 1.0f));
        ...
        private class DefinitionAdjuster extends Adjuster<GPUImageDefinitionFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setExposure(range(percentage, -0.3f, 1.0f));

          /*      if(percentage<100){
                    getFilter().setExposure(range(percentage, -0.3f, 0f));
                }
                else{
                    getFilter().setExposure(range(percentage, -0f, 1.0f));
                }*/
        ...
        private class HighBcnetechAdjuster extends Adjuster<GPUImageHighBcnetechFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setHighlights(range(percentage, -1.0f, 1.0f));
            }
            @Override
            public void adjust(int percentage, int type) {
        ...
        private class ShadowBcnetechAdjuster extends Adjuster<GPUImageShadowBcnetechFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setShadows(range(percentage, -1.0f, 1.0f));
            }
            @Override
            public void adjust(int percentage, int type) {
