...
//            wrapAdapter.notifyItemChanged(selectPos + 1);
    ...
    public void onScrolled(int dx, int dy) {
        ...
        int itemWidth = wrapAdapter.getItemWidth();
        deltaX += dx;
        float newPos=initPos+(float)deltaX/(float)itemWidth;

        float c=Math.abs((float)deltaX/(float)itemWidth);
        if((int)newPos==initPos){
            mainPos=initPos;
            targetPos=initPos-1;
        } else {

            mainPos=(int)newPos+1;
            targetPos=(int)newPos;
        }

        if(null!=listener){
            if(c-(int)c==0&&(int)newPos!=initPos){
                listener.scrolledPositionChanged(mainPos,targetPos,(c-(int)c),1-(c-(int)c),(int)newPos,c);
            }else {
                listener.scrolledPositionChanged(mainPos,targetPos,1-(c-(int)c),(c-(int)c),(int)newPos,c);
            }
        }
    ...
    class WrapperAdapter extends RecyclerView.Adapter {
        ...
        public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            if (viewType == HEADER_FOOTER_TYPE) {
                ...
                headerFooterWidth = parent.getMeasuredWidth() / 2 - (ImageUtil.Dp2Px(context,52))/2 ;
            ...
            int width =ImageUtil.Dp2Px(context,52);
    ...
    public interface OnSelectedPositionChangedListener {
        ...
        void scrolledPositionChanged(int mainPos,int targetPos,float mainPercent,float targetPercent,int selectPos,float selectPercent);
    }
