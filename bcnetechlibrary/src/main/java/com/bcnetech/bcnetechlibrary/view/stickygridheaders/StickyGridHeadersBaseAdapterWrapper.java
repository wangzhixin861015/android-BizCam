...
package com.bcnetech.bcnetechlibrary.view.stickygridheaders;
...
public class StickyGridHeadersBaseAdapterWrapper extends BaseAdapter {
    ...
    protected static final int ID_HEADER_LAYOUT = -0x04;

    protected static final int ID_HEADER_LAYOUT_FILLER = -0x05;

    protected static final int ID_FOOTER_LAYOUT = -0x06;

    protected static final int ID_FOOTER_LAYOUT_FILLER = -0x07;
    ...
    protected static final int POSITION_HEADER_LAYOUT = -0x04;

    protected static final int POSITION_HEADER_LAYOUT_FILLER = -0x05;

    protected static final int POSITION_FOOTER_LAYOUT = -0x06;

    protected static final int POSITION_FOOTER_LAYOUT_FILLER = -0x07;
    ...
    protected static final int VIEW_TYPE_HEADER_LAYOUT = -0x03;

    protected static final int VIEW_TYPE_HEADER_LAYOUT_FILLER = 0x04;

    protected static final int VIEW_TYPE_FOOTER_LAYOUT = 0x05;

    protected static final int VIEW_TYPE_FOOTER_LAYOUT_FILLER = 0x06;
    ...
    private View mLastHeaderLayoutViewSeen;

    private View mLastFooterLayoutViewSeen;
    ...
    public int getCount() {
        ...
        mCounted = true;
        if(mGridView.getFooterLayout()!=null){
            mCount+=mNumColumns;
        }
        if(mGridView.getHeaderlayout()!=null){
            mCount+=mNumColumns;
        }
        return mCount;
    }
    ...
    public Object getItem(int position) throws ArrayIndexOutOfBoundsException {
        ...
        if (adapterPosition.mPosition == POSITION_FILLER
                ...
                || adapterPosition.mPosition == POSITION_HEADER_FILLER
                || adapterPosition.mPosition == POSITION_HEADER_LAYOUT
                || adapterPosition.mPosition == POSITION_HEADER_LAYOUT_FILLER
                || adapterPosition.mPosition == POSITION_FOOTER_LAYOUT
                || adapterPosition.mPosition == POSITION_FOOTER_LAYOUT_FILLER) {
            // Fake entry in view.
            return null;
        }
    ...
    public long getItemId(int position) {
        ...
        if (adapterPosition.mPosition == POSITION_HEADER_LAYOUT) {
            return ID_HEADER_LAYOUT;
        }
        if (adapterPosition.mPosition == POSITION_HEADER_LAYOUT_FILLER) {
            return ID_HEADER_LAYOUT_FILLER;
        }
        if (adapterPosition.mPosition == POSITION_FOOTER_LAYOUT) {
            return ID_FOOTER_LAYOUT;
        }
        if (adapterPosition.mPosition == POSITION_FOOTER_LAYOUT_FILLER) {
            return ID_FOOTER_LAYOUT_FILLER;
        }
    ...
    public int getItemViewType(int position) {
        ...
        if (adapterPosition.mPosition == POSITION_HEADER_LAYOUT) {
            return VIEW_TYPE_HEADER_LAYOUT;
        }
        if (adapterPosition.mPosition == POSITION_HEADER_LAYOUT_FILLER) {
            return VIEW_TYPE_HEADER_LAYOUT_FILLER;
        }
        if (adapterPosition.mPosition == POSITION_FOOTER_LAYOUT) {
            return VIEW_TYPE_FOOTER_LAYOUT;
        }
        if (adapterPosition.mPosition == POSITION_FOOTER_LAYOUT_FILLER) {
            return VIEW_TYPE_FOOTER_LAYOUT_FILLER;
        }
        ...
        return itemViewType + sNumViewTypes
                +(mGridView.getHeaderlayout()==null?0:2)
                +(mGridView.getFooterLayout()==null?0:2);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (adapterPosition.mPosition == POSITION_HEADER) {
        ...
        }else if (adapterPosition.mPosition == POSITION_HEADER_LAYOUT) {
            HeaderFillerView v = getHeaderFillerView(adapterPosition.mHeader, convertView, parent);
            View view = mGridView.getHeaderlayout();
            mGridView.detachHeader((View) v.getTag());
            v.setTag(view);
            mGridView.attachHeader(view);
            convertView = v;
            mLastHeaderLayoutViewSeen = v;
            v.forceLayout();
        } else if (adapterPosition.mPosition == POSITION_HEADER_LAYOUT_FILLER) {
            convertView = getFillerView(convertView, parent, mLastHeaderLayoutViewSeen);
            convertView.forceLayout();
        }else if (adapterPosition.mPosition == POSITION_FOOTER_LAYOUT) {
            HeaderFillerView v = getHeaderFillerView(adapterPosition.mHeader, convertView, parent);
            View view = mGridView.getHeaderlayout();
            mGridView.detachHeader((View) v.getTag());
            v.setTag(view);
            mGridView.attachHeader(view);
            convertView = v;
            mLastFooterLayoutViewSeen = v;
            v.forceLayout();
        } else if (adapterPosition.mPosition == POSITION_FOOTER_LAYOUT_FILLER) {
            convertView = getFillerView(convertView, parent, mLastFooterLayoutViewSeen);
            convertView.forceLayout();
    ...
    protected Position translatePosition(int position) {
        ...
        if(mGridView.getFooterLayout()!=null){
            if(mCount-position<=mNumColumns){
                int place =mNumColumns-(mCount-position);
                int i;
                for (i = 0; i < mNumColumns; i++) {
                    if (place == 0) {
                        // Position is first column where header will be.
                        return new Position(POSITION_FOOTER_LAYOUT, i);
                    }
                    place -= mNumColumns;
                    if (place < 0) {
                        // Position is a fake so return null.
                        return new Position(POSITION_FOOTER_LAYOUT_FILLER, i);
                    }
                }
        ...
        if(mGridView.getHeaderlayout()!=null){
            if(position>=mNumColumns){
                ...
                for (i = 0; i < mNumColumns; i++) {
                    if (place == 0) {
                        // Position is first column where header will be.
                        return new Position(POSITION_HEADER_LAYOUT, i);
                    }
                    place -= mNumColumns;
                    if (place < 0) {
                        // Position is a fake so return null.
                        return new Position(POSITION_HEADER_LAYOUT_FILLER, i);
                    }
                }
            }
        }
