...
public abstract class BaseObserver<T> implements Observer<BaseResult<T>> {
    ...
    public void onNext(BaseResult<T> tBaseEntity) {
        ...
        if (tBaseEntity.isSuccess()) {
            ...
            try {
                if(tBaseEntity.getCode()==10010){
                    onTokenError(tBaseEntity);
                }else {
...
//        Log.w(TAG, "onError: ", );这里可以打印错误信息
     ...
     * @throws Exception
     */
    protected abstract void onCodeError(BaseResult<T> t) throws Exception ;
     ...
     * @throws Exception
     */
    protected abstract void onTokenError(BaseResult<T> t) throws Exception ;
